...
export const selfEvolveConfigSchema = {
  parse(value: unknown): SelfEvolveConfig {
    ...
    const k1 = Math.floor(
      readNumber(retrievalRaw, "k1", DEFAULT_CONFIG.retrieval.k1, 1, 100),
    );
    const k2 = Math.floor(
      readNumber(retrievalRaw, "k2", DEFAULT_CONFIG.retrieval.k2, 1, 20),
    );
    if (k2 > k1) {
    ...
    return {
      ...
      reward: {
        ...
        temperature: readNumber(
          rewardRaw,
          "temperature",
          DEFAULT_CONFIG.reward.temperature,
          0,
          1,
        ),
      },
      runtime: {
        minPromptChars: Math.floor(
          readNumber(
            runtimeRaw,
            "minPromptChars",
            DEFAULT_CONFIG.runtime.minPromptChars,
            1,
            200,
          ),
        ),
